...
static int output_file_init(struct output_file *out, int block_size,
		int64_t len, bool sparse, int chunks, bool crc, int enablePad, unsigned char padByte)
// MStar Android Patch End
{
	...
	// MStar Android Patch Begin
	// Pre fill the pad data if padding enable
	else if(enablePad){
            unsigned char padBlock[64*1024];
            memset(padBlock, padByte, 64*1024);
            while(len>=64*1024)
            {
                out->ops->write(out, padBlock, 64*1024);
                len -= 64*1024;
            }
            if(len>0){
                out->ops->write(out, padBlock, len);
            }
            out->ops->skip(out, -out->len);
	}
	// MStar Android Patch End
		...
		int chunks, int crc)
{
	...
	outc->write = write;
	// MStar Android Patch Begin
	// Only make_ext4fs utility use the padding
	ret = output_file_init(&outc->out, block_size, len, sparse, chunks, crc, 0, 0);
	// MStar Android Patch End
	if (ret < 0) {
		free(outc);
		return NULL;
...
struct output_file *output_file_open_fd(int fd, unsigned int block_size, int64_t len,
		int gz, int sparse, int chunks, int crc, int enablePad, unsigned char padByte)
// MStar Android Patch End
{
	int ret;
	struct output_file *out;
	...
	// MStar Android Patch Begin
	// Add argus for pad
	ret = output_file_init(out, block_size, len, sparse, chunks, crc, padByte, enablePad);
	// MStar Android Patch End
	if (ret < 0) {
		free(out);
		return NULL;
