...
// MStar Android Patch Begin
// Record the max block size and pad here.
int sparse_file_add_data(struct sparse_file *s,
		void *data, unsigned int len, unsigned int block)
{
	int ret;

	ret = backed_block_add_data(s->backed_block_list, data, len, block);
	if (block + DIV_ROUND_UP(len, s->block_size) > aux_info.max_block) {
		aux_info.max_block = block + DIV_ROUND_UP(len, s->block_size);
		aux_info.max_len = (int64_t)block * info.block_size + (int64_t)ALIGN(len, s->block_size);
	}
		...
		uint32_t fill_val, unsigned int len, unsigned int block)
{
	...
	ret = backed_block_add_fill(s->backed_block_list, fill_val, len, block);
	if (block + DIV_ROUND_UP(len, s->block_size) > aux_info.max_block) {
		aux_info.max_block = block + DIV_ROUND_UP(len, s->block_size);
		aux_info.max_len = (int64_t)block * info.block_size + (int64_t)ALIGN(len, s->block_size);
	}
		...
		unsigned int block)
{
	...
	ret = backed_block_add_file(s->backed_block_list, filename, file_offset,
			len, block);
	if (block + DIV_ROUND_UP(len, s->block_size) > aux_info.max_block) {
		aux_info.max_block = block + DIV_ROUND_UP(len, s->block_size);
		aux_info.max_len = (int64_t)block * info.block_size + (int64_t)ALIGN(len, s->block_size);
		...
		int fd, int64_t file_offset, unsigned int len, unsigned int block)
{
	...
	ret = backed_block_add_fd(s->backed_block_list, fd, file_offset,
			len, block);
	if (block + DIV_ROUND_UP(len, s->block_size) > aux_info.max_block) {
		aux_info.max_block = block + DIV_ROUND_UP(len, s->block_size);
		aux_info.max_len = (int64_t)block * info.block_size + (int64_t)ALIGN(len, s->block_size);
...
int sparse_file_write(struct sparse_file *s, int fd, bool gz, bool sparse,
		bool crc, unsigned char padByte, int enablePad)
{
	int ret;
	int chunks;
	...
	// MStar Android Patch Begin
	// Use the max data len instead of file size(s->len)
	out = output_file_open_fd(fd, s->block_size, aux_info.max_len/*s->len&*/, gz, sparse, chunks, crc, padByte, enablePad);
	// MStar Android Patch End
	if (!out)
		return -ENOMEM;
