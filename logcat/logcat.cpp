    ...
    while (1) {
        do {
            ...
            if (g_kmsg) {
                ...
                if ((ret = klogctl(KLOG_SIZE_UNREAD, kernel_buffer, sizeof(kernel_buffer))) > 0) {
                   if ((ret = klogctl(KLOG_READ, kernel_buffer, sizeof(kernel_buffer))) > 0) {
                       if (printKernelBuffer(kernel_buffer, ret) < 0) {
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        // MStar Android Patch Begin
        ret = getopt(argc, argv, "cdt:gsQf:r::n:v:b:BK");
        // MStar Android Patch End
...
#define  CONSOLE_OPTION "androidboot.console="
    ...
    if (android::g_kmsg) {
        char *kernel_buffer;
        int ret, klog_buf_len;
        klog_buf_len = klogctl(KLOG_SIZE_UNREAD, 0, 0);
        if (klog_buf_len == 0) {
            /*
            ...
            */
            ...
            kernel_buffer = (char *)malloc(klog_buf_len + 1);
            if ((ret = klogctl(KLOG_READ_ALL, kernel_buffer, klog_buf_len)) > 0) {
                kernel_buffer[ret] = '\n';
                if (android::printKernelBuffer(kernel_buffer, ret) < 0) {
                    perror ("write kernel log error");
                }
            }
        } else {
            kernel_buffer = (char *)malloc(klog_buf_len + 1);
            if ((ret = klogctl(KLOG_READ, kernel_buffer, klog_buf_len)) > 0) {
                kernel_buffer[ret] = '\n';
                if (android::printKernelBuffer(kernel_buffer, ret) < 0) {
                    perror ("write kernel log error");
                }
