...
static void handle_generic_device_event(struct uevent *uevent)
{
    ...
    if (!strncmp(uevent->subsystem, "usb", 3) || !strncmp(uevent->subsystem, "usbmisc", 7)) {
...
// MStar Android Patch Begin
// Hardcode mstar device node.
static void mstarboot(void)
{
    mknod("/dev/ge",        0660 | S_IFCHR, (135 << 8) | (0));
    mknod("/dev/gflip",     0660 | S_IFCHR, (234 << 8) | (0));
    mknod("/dev/gop",       0660 | S_IFCHR, (136 << 8) | (0));
    mknod("/dev/nodev",     0660 | S_IFCHR, (176 << 8) | (0));
    mknod("/dev/msmailbox", 0660 | S_IFCHR, (175 << 8) | (0));
    mknod("/dev/system",    0660 | S_IFCHR, (148 << 8) | (0));
    mknod("/dev/smart",     0660 | S_IFCHR, (146 << 8) | (0));
    mknod("/dev/ir",        0660 | S_IFCHR, (156 << 8) | (0));
    mknod("/dev/idong0",    0660 | S_IFCHR, (180 << 8) | (192));

    chown("/dev/ge", AID_SYSTEM, AID_SYSTEM);
    chown("/dev/gflip", AID_SYSTEM, AID_SYSTEM);
    chown("/dev/gop", AID_SYSTEM, AID_SYSTEM);
    chown("/dev/nodev", AID_SYSTEM, AID_SYSTEM);
    chown("/dev/msmailbox", AID_MEDIA, AID_SYSTEM);
    chown("/dev/system", AID_MEDIA, AID_SYSTEM);
    chown("/dev/smart", AID_SYSTEM, AID_SYSTEM);
    chown("/dev/ir", AID_SYSTEM, AID_SYSTEM);
    chown("/dev/idong0", AID_SYSTEM, AID_SYSTEM);
}
// MStar Android Patch End
...
void device_init(void)
{
    ...
    if (stat(coldboot_done, &info) < 0) {
        ...
        // MStar Android Patch Begin
        // Hardcode mstar device node.
        // FIXME: should not hardcode to create nodes for MStar drivers.
        mstarboot();
        // MStar Android Patch End
        t1 = get_usecs();
