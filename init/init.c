...
// Refine init action and auto reboot when zygote fail
...
static char reproducerate[32];
#endif
static char envtype[32];
// MStar Android Patch End
...
int add_environment(const char *key, const char *val)
{
    ...
    // MStar Android Patch Begin
    for (n = 0; n < 31; n++) {
        // check if we should overwrite the previous same environment var
        if (ENV[n] != NULL) {
            size_t key_len = strchr(ENV[n], '=') - ENV[n];

            if (key_len == strlen(key) && strncmp(ENV[n], key, key_len) == 0) {
                free(ENV[n]);
                ENV[n] = NULL;
            } else {
                continue;
        ...
        // set a new environment var OR overwrite the previous same environment var
...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    // MStar Android Patch Begin
    // Add boot time check point
    if (strcmp(svc->name,"tvservice")==0) {
        ERROR("boot time check(need add timer gap): start '%s'\n",svc->name);
    }
    // MStar Android Patch End
...
static void restart_service_if_needed(struct service *svc)
{
    ...
    if (next_start_time <= gettime()) {
        ...
        if (strcmp(svc->name,"zygote") == 0) {
            if (need_reboot >= REBOOT_COUNT) {
                ERROR(" !!! Zygote Restart More than %d times , Reboot System!!!\n", need_reboot);
                ERROR(" !!! Going to Reboot !!!");
                reboot(RB_AUTOBOOT);
            } else {
                need_reboot++;
            }
...
// MStar Android Patch Begin
#ifdef BUILD_FOR_STB
    property_set("mstar.resolution", resolution);
    property_set("mstar.reproducerate", reproducerate);
#endif
    property_set("mstar.envtype",envtype);
// MStar Android Patch End
}
